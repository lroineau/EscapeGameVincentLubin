...
function distance(depart,arrivee) {
    // renvoie la distance en km entre deux points (sous forme d'objets leaflet)
...
//marche pas pour la même raison que fzoom
function relachement(e) {
    marqueur.getPopup().setContent(''+marqueur.getLatLng());
    marqueur.openPopup();
}
...
//************************************************************************************************
// Fonction qui ajoute un marqueur
function marqueur(Object){
  ...
  var icon = L.icon({
      ...
      iconSize:     [Object['taillex'],Object['tailley']], // size of the icon
      iconAnchor:   [20,80], // point of the icon which will correspond to marker's location
      popupAnchor:  [40,-80] // point from which the popup should open relative to the iconAnchor
  });
      ...
      .then(result => {
           ...
           document.getElementById("instruction_jeu").innerHTML=inst;
            ...
            if (fonctionnalite=='normal'){
              ...
              //questionnaire_geodesie();
              // inventaire[n].getPopup().setContent(template);
